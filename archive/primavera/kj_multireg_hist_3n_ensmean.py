...
#### costruire alldrivs e gigi con solo nome modello
alldrivs_em = dict()
for mod in modgen_all:
    tut = [co for co in models_ok_all if co.split('_')[0] == mod]
    for ci in drilis_all:
        spi = [alldrivs[(ci[0], ci[1], tuno)] for tuno in tut]
        alldrivs_em[(ci[0], ci[1], mod)] = np.nanmean(spi)
...
for mod in modgen_all:
    ...
    tut = [co for co in models_all if co.split('_')[0] == mod]
    for reg in regtip:
        gigi_em[mod][reg] = dict()
        for metr in metrics[reg]:
            spi = [gigi[tuno][reg][metr[0]][metr[1]] for tuno in tut]
            gigi_em[mod][reg][metr] = np.nanmean(spi)
...
def make_XY(reg, comb, modgen = modgen_all, alldrivs = alldrivs_em, gigi = gigi_em, drilis = drilis_all, metrics = metrics, standard_Y = True):
    Y = []
    for mod in modgen:
        Y.append([gigi[mod][reg][metr] for metr in metrics[reg]])
    Y = np.stack(Y)
...
for modgen, ensmod in zip([modgen_all], ['all']):
    ...
    for nu, colnu in zip(np.arange(2, 7), ctl.color_set(6)):
        ...
        for reg in regtip:
            ...
            for ii, comb in enumerate(allcombs):
                ...
                if nu < 7 and len(np.unique(tipik)) < len(tipik): # single driver from each family
