...
radclim = dict()
ensmems = ['lab{}'.format(i) for i in range(10)] + ['las{}'.format(i) for i in range(10)]
for exp in ensmems:
...
for cos in ['map', 'map_std', 'zonal', 'zonal_std', 'global', 'global_std']:
    for am in annme:
        for key in varnames+['toa_balance']+['surf_balance']:
            radclim[('base', cos, key, am)] = np.mean([radclim[(ens, cos, key, am)] for ens in ensmems[:10]], axis = 0)
            radclim[('stoc', cos, key, am)] = np.mean([radclim[(ens, cos, key, am)] for ens in ensmems[10:]], axis = 0)
