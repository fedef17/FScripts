...
cart_trends = cart_out + '../trends/'
trendz = pickle.load(open(cart_trends + 'trendz_1000.p', 'rb'))

# map of temp trends
#rat1 = patt_maps[('tas', exp2, 'stab')]/patt_maps[('tas', 'ssp585', 'fin')]
#rat2 = patt_maps[('tas', exp1, 'stab')]/patt_maps[('tas', 'ssp585', 'fin')]
rat1 = trendz[('tas', exp1)]/trendz[('tas', 'ssp585')]
rat2 = trendz[('tas', exp2)]/trendz[('tas', 'ssp585')]
rat3 = trendz[('tas', exp2)]/trendz[('tas', exp1)]

divnorm = mcolors.TwoSlopeNorm(vmin=0.05, vcenter=1., vmax=2.75)
ctl.plot_multimap_contour([rat1, rat2], coso.lat, coso.lon, visualization = 'Robinson', subtitles = ['{}/ssp585'.format(exp1), '{}/ssp585'.format(exp2)], cbar_range = (0.05, 2.75), color_norm = divnorm, cmap = ctl.heatmap(), filename = cart_out + 'tas_patt_NEW_p2_trendz.pdf', figsize = (16,7), cb_label = 'Ratio of warming patterns', cbar_bottomspace = 0.11, n_color_levels = 28, draw_grid = True)

ctl.plot_multimap_contour([rat1, rat3], coso.lat, coso.lon, visualization = 'Robinson', subtitles = ['{}/ssp585'.format(exp1), '{}/{}'.format(exp2, exp1)], cbar_range = (0.05, 2.75), color_norm = divnorm, cmap = ctl.heatmap(), filename = cart_out + 'tas_patt_NEW_p3_trendz.pdf', figsize = (16,7), cb_label = 'Ratio of warming patterns', cbar_bottomspace = 0.11, n_color_levels = 28, draw_grid = True)


#### TEMPERATURE patt map with only ssp585, b025, b100
cmap = ctl.heatmap()
cbran = (0.25, 2.95)
divnorm = mcolors.TwoSlopeNorm(vmin=0.25, vcenter=1., vmax=2.95)
plotanom = False
cblab = 'Temperature change per degree of global warming'
okfigs = [trendz[('tas', exp)] for exp in ['ssp585', exp1, exp2]]
okha = [trendz[('tas', exp, 'pval')] < 0.05 for exp in ['ssp585', exp1, exp2]]

ctl.plot_multimap_contour(okfigs, coso.lat, coso.lon, filename = cart_out + 'tas_patt_NEW_p1_trendz.pdf', cmap = cmap, plot_anomalies = plotanom, cbar_range = cbran, figsize = (18, 7), subtitles = ['ssp585', exp1, exp2], color_norm = divnorm, cb_label = cblab, visualization = 'Robinson', central_lat_lon = (0.,0.), n_color_levels = 28, fix_subplots_shape = (1, 3), draw_grid = True, add_hatching = okha, hatch_styles = ['////', '', ''])


#### PRECIPITATION Patt map with only ssp585, b025, b100
c5 = -11
c95 = 21
...
okfigs = [trendz[('pr_rel', exp)] for exp in ['ssp585', exp1, exp2]]
okha = [trendz[('pr_rel', exp, 'pval')] < 0.05 for exp in ['ssp585', exp1, exp2]]

ctl.plot_multimap_contour(okfigs, coso.lat, coso.lon, filename = cart_out + 'pr_patt_NEW_p1_trendz.pdf', cmap = cmap, plot_anomalies = plotanom, cbar_range = cbran, figsize = (18, 7), subtitles = ['ssp585', exp1, exp2], color_norm = divnorm, cb_label = cblab, visualization = 'Robinson', central_lat_lon = (0.,0.), n_color_levels = 17, fix_subplots_shape = (1, 3), draw_grid = True, add_hatching = okha, hatch_styles = ['////', '', ''])
...
for ru, ax, ax2 in zip([exp1, exp2], axs, axs2):
    ziup = trendz[('pr_rel', ru)]
    piuk = trendz[('pr_rel', 'ssp585')]
...
fig2.savefig(cart_out + 'prssp_{}_{}_scatter_trendz.png'.format(exp1, exp2))
fig.savefig(cart_out + 'map_prssp_{}_{}_scatter_trendz.png'.format(exp1, exp2))
fig.savefig(cart_out + 'map_prssp_{}_{}_scatter_trendz.pdf'.format(exp1, exp2))
